(function () {
  function initBank() {
    ...
    if (!panel || !subtabs || !picked || !baseInput || !opsTotal || !afterTotal || !pickedCount || !manualText || !manualValue) return;
    ...
    function getSections() {
      ...
      if (state.sub[state.main] === 'all') return arr;

      var out = [];
      for (var i = 0; i < arr.length; i++) {
        if (arr[i].id === state.sub[state.main]) out.push(arr[i]);
      }
      return out;
    }

    function titleHtml(item) {
      if (item.url) return '<a href="' + esc(item.url) + '">' + esc(item.text) + '</a>';
    ...
    function selectedItems() {
      ...
      for (var m = 0; m < state.manual.length; m++) out.push(state.manual[m]);
      return out;
    }
    ...
    function opsSum() {
      ...
      var total = 0;
      for (var i = 0; i < list.length; i++) total += Number(list[i].sum || 0);
      return total;
    }
    ...
    function renderTabs() {
      ...
      for (var i = 0; i < mains.length; i++) {
        mains[i].classList.toggle('is-active', mains[i].getAttribute('data-main') === state.main);
      }
    ...
    function renderPanel() {
      ...
      for (var i = 0; i < secs.length; i++) {
        ...
        if (sec.note) html += '<div class="cdb2__sectionpill">' + esc(sec.note) + '</div>';
        html += '</div>';
        ...
        for (var j = 0; j < sec.items.length; j++) {
          ...
          if (item.manual) {
            ...
            html += '<div><div class="cdb2__rowtitle">' + titleHtml(item) + '</div>';
            html += '<div class="cdb2__notetext">это не считается автоматически — добавь ниже вручную</div></div>';
            ...
            html += '<div class="cdb2__rowmain"><div class="cdb2__rowtitle">' + titleHtml(item) + '</div></div>';
    ...
    function renderPicked() {
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        html += '<div class="cdb2__pickedtext">' + esc(row.text);
        html += '<small>' + esc(row.group || 'Ручная операция') + (row.qty ? ' × ' + row.qty : '') + '</small></div>';
    ...
    function copyReport() {
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        if (line.manual) manual.push(rowText);
        else if (line.side === 'earn') earn.push(rowText);
        else spend.push(rowText);
      }
    ...
    root.addEventListener('click', function (e) {
      ...
      if (act === 'addManualPlus') return addManual(1);
      if (act === 'addManualMinus') return addManual(-1);
      if (act === 'clear') return clearOps();
