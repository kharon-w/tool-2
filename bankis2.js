(function(){
  ...
  for (var i = 0; i < dataBlocks.length; i++) {
    ...
    var title = box.getAttribute('data-title') || 'Раздел';
    ...
    for (var j = 0; j < lines.length; j++) {
      ...
      var isNumeric = /^-?\d+([.,]\d+)?$/.test(raw);
      var manual = mode === 'manual' || !isNumeric;
      var value = manual ? 0 : Number(raw.replace(',', '.'));
      var id = 'cdb2_' + (++uid);
  ...
  function addManual(sign){
    ...
    var value = Number(manualValue.value || 0);
  ...
  function copyReport(){
    ...
    for (var i = 0; i < list.length; i++) {
      var line = list[i];
      var rowText = (line.sum >= 0 ? '+ ' : '- ') + format(Math.abs(line.single || Math.abs(line.sum))) + currency + ' — ' + line.text + (line.qty ? ' × ' + line.qty : '') + ' = ' + format(Math.abs(line.sum)) + currency;
  ...
  root.addEventListener('click', function(e){
    ...
    if (mainTab) {
      state.main = mainTab.getAttribute('data-main');
      renderTabs();
      renderPanel();
      return;
    }
    ...
    if (subTab) {
      state.sub[state.main] = subTab.getAttribute('data-sub');
      renderTabs();
      renderPanel();
      return;
    }
  ...
  root.addEventListener('input', function(e){
    ...
    if (e.target === baseInput) {
      state.base = Number(baseInput.value || 0);
      updateTotals();
    }
  });
